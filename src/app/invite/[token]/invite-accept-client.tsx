...
export default function InviteAcceptClient({ token, invitedEmail, clientName, clientId }: Props) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      // 서버사이드 API로 유저 생성 (service role 사용, 이메일 인증 자동 처리)
      const res = await fetch("/api/agency/invite-client/accept", {
        method: "POST",
    ...
    <div className="min-h-screen flex items-center justify-center p-4 bg-background">
      <Card className="w-full max-w-md">
        <CardHeader className="text-center">
          ...
          <CardTitle>포털 초대를 수락했습니다</CardTitle>
          <CardDescription>
            <strong>{clientName}</strong>의 클라이언트 포털에 접속하려면 계정을 설정해 주세요.
          </CardDescription>
        </CardHeader>
        <CardContent>
