...
# 앱 URL
# · 로컬: 아래처럼 localhost 유지 (.env.local)
# · Vercel 배포: 대시보드에서 별도 설정 (아래 "로컬 vs 배포" 참고)
...
# Google Ads API용 (선택). 넣어두면 관리자 "연동 기본설정"에서 매번 입력 안 해도 됨
GOOGLE_DEVELOPER_TOKEN=    # [Google Ads](https://ads.google.com) → 도구 및 설정 → API 센터에서 발급

# ── Google Gemini AI (보고서 자동 생성) ──
GEMINI_API_KEY=            # Google AI Studio에서 발급
```

### 1-1. 로컬 vs Vercel 배포 — `NEXT_PUBLIC_APP_URL` 구분

| 환경 | 설정 위치 | 값 |
|------|------------|-----|
| **로컬** | 프로젝트 루트 `.env.local` | `NEXT_PUBLIC_APP_URL=http://localhost:3000` |
| **Vercel (배포)** | Vercel 대시보드 → 프로젝트 → **Settings** → **Environment Variables** | `NEXT_PUBLIC_APP_URL=https://www.onemarketing.kr` (끝에 슬래시 없음) |
...
3. **Key**: `NEXT_PUBLIC_APP_URL`  
   **Value**: `https://www.onemarketing.kr`  
   (프로토콜 포함, 끝에 슬래시 없음)
4. **Environment**: Production(필수), Preview는 필요 시 체크
5. 저장 후 다음 배포부터 적용 (이미 배포된 건 **Redeploy** 해야 반영)

이렇게 하면 **로컬**에서는 `http://localhost:3000`으로 OAuth 콜백·API 기준 URL이 잡히고, **배포(www.onemarketing.kr)** 에서는 `https://www.onemarketing.kr`로 잡힙니다. Meta/Google OAuth 앱의 "승인된 리디렉션 URI"에는 **두 URL 모두** 등록해 두어야 로컬·배포 둘 다 로그인됩니다.

### 1-2. 관리자 "연동 기본설정"에 넣을 값

관리자 → 클라이언트 → [클라이언트 선택] → **데이터 연동** 탭 → **연동 기본설정**에서 아래 값을 채웁니다.  
(DB `admin_settings`에 저장되며, 없으면 `.env` 값을 씁니다.)

| 항목 | 넣을 값 | 비고 |
|------|---------|------|
| **Meta App ID** | `.env.local`의 `META_APP_ID`와 동일 | 이미 env에 있으면 비워도 됨(env 사용) |
| **Meta App Secret** | `.env.local`의 `META_APP_SECRET`과 동일 | 위와 동일 |
| **Google Client ID** | `.env.local`의 `GOOGLE_CLIENT_ID`와 동일 | 위와 동일 |
| **Google Client Secret** | `.env.local`의 `GOOGLE_CLIENT_SECRET`과 동일 | 위와 동일 |
| **Google Developer Token (선택)** | [Google Ads](https://ads.google.com) → 도구 및 설정 → **API 센터**에서 발급한 토큰 | `.env`에 `GOOGLE_DEVELOPER_TOKEN` 넣어두면 여기선 비워도 됨. 여기 한 번 넣어두면 연동마다 입력 안 해도 됨 |
| **앱 URL (OAuth 콜백용)** | **로컬에서 테스트할 때:** `http://localhost:3000` (끝에 슬래시 없음) / **배포 서버에서 쓸 때:** `https://www.onemarketing.kr` (끝에 슬래시 없음) | 지금 접속한 환경에 맞춰 넣기. 로컬에서 저장 시 localhost, 배포 후 관리자 접속 시에는 https://www.onemarketing.kr |

- **앱 URL:** 로컬 개발 시에는 `http://localhost:3000`, 배포 도메인은 `https://www.onemarketing.kr` 로 통일.
- **Google Developer Token:** env에 없으면 "연동 기본설정"에 한 번 넣어두면 Google Ads 연동 시 매번 입력하지 않아도 됩니다.
...
## 2-2. 한 번만 vs 고객(클라이언트)마다 — 꼭 구분해서 보기

**고객이 들어올 때마다 Meta 앱·Google 프로젝트를 새로 만드는 것이 아닙니다.**

| 구분 | 한 번만 (SaaS 운영자 = 당신) | 고객(클라이언트) 추가될 때마다 |
|------|-----------------------------|--------------------------------|
| **Meta** | Meta 앱 **1개** 생성 → `META_APP_ID`, `META_APP_SECRET`을 `.env`에 **1세트**만 설정 | 해당 고객(클라이언트) 상세 → 데이터 연동에서 **"Meta OAuth" 버튼** 클릭 → 고객(또는 대행사)이 자기 Facebook으로 로그인해 연결. **앱을 또 만드는 작업 없음** |
| **Google** | Google Cloud에서 OAuth 클라이언트 **1개** 생성 → `GOOGLE_CLIENT_ID`, `GOOGLE_CLIENT_SECRET`을 `.env`에 **1세트**만 설정 | 해당 고객 상세 → 데이터 연동에서 **"Google OAuth" 버튼** 클릭 → 고객이 자기 Google로 로그인해 연결. **프로젝트/클라이언트 ID를 또 만드는 작업 없음** |
| **네이버 검색광고** | (네이버는 “앱” 개념 없음. 라이선스만 발급) | 그 **고객(또는 그 고객의 검색광고 계정)** 이 네이버에서 API 라이선스 1개 발급 → 관리자가 그 클라이언트의 데이터 연동 탭에 **API Key, Secret Key, Customer ID만 입력** |

**정리**

- **Meta / Google**: 위 표의 “한 번만” 작업을 끝내 두면, 이후에는 **고객 추가 시 관리자 페이지에서 OAuth 버튼만 눌러서 연결**하면 됩니다. 새 앱·새 프로젝트는 만들지 않습니다.
- **네이버**: 고객(또는 해당 검색광고 계정 권한자)이 라이선스 1개 발급 → 발급받은 3개 값만 포털에 입력하는 작업이 **고객당 1번**입니다.
...
| "enabled_services" / "Could not find the 'enabled_services' column" | 컬럼 미추가 또는 스키마 캐시 | Supabase SQL Editor에서 `supabase/sql/04-migrations/003-enabled-services-schema-reload.sql` 실행 (컬럼 추가 + `NOTIFY pgrst, 'reload schema';`) |
...
### 7-1. 솔라피(Solapi) 가입 및 API Key 발급

| 항목 | 설명 |
|------|------|
| **발급 위치** | [솔라피](https://solapi.com) |
...
3. 솔라피 콘솔에서 카카오톡 채널 연동
4. 프로필 ID(PFID) 확인 → 환경 변수 설정:
   ```bash
...
카카오 비즈센터에서 아래 템플릿을 **사전 등록 및 승인**받아야 합니다:
...
> ⚠️ 템플릿 승인까지 1~2영업일 소요됩니다.
> 템플릿 ID는 `src/lib/notifications/alimtalk.ts`의 `TEMPLATE_IDS`에서 관리합니다.

### 7-4. 테스트

1. 관리자 → 알림 설정 페이지 접속
2. 전화번호가 등록된 클라이언트 선택
3. 알림 유형 선택 → "테스트 발송" 클릭
4. 발송 결과 확인
...
src/
  ...
  admin/notifications/         → 알림 설정 관리 페이지
  lib/notifications/alimtalk.ts → 카카오 알림톡 발송 모듈
  api/admin/notifications/send/ → 알림 발송 API
