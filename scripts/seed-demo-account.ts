/**
 * 심사용 테스트 계정 + 목업 데이터 1회 생성
 * 실행: npx tsx scripts/seed-demo-account.ts
 * 필요 env: NEXT_PUBLIC_SUPABASE_URL, SUPABASE_SERVICE_ROLE_KEY
 * 선택 env: DEMO_ACCOUNT_EMAIL, DEMO_ACCOUNT_PASSWORD (없으면 아래 기본값 사용)
 */
...
const DEFAULT_DEMO_EMAIL = "demo@gmail.com";
...
const email = (process.env.DEMO_ACCOUNT_EMAIL ?? DEFAULT_DEMO_EMAIL).trim().toLowerCase();
const password = process.env.DEMO_ACCOUNT_PASSWORD ?? DEFAULT_DEMO_PASSWORD;

if (!url || !serviceKey) {
  console.error("NEXT_PUBLIC_SUPABASE_URL, SUPABASE_SERVICE_ROLE_KEY 가 필요합니다. .env.local 또는 .env.seed 또는 환경 변수를 설정하세요.");
  process.exit(1);
}
...
async function main() {
  ...
  if (authErr) {
    ...
    if (authErr.status === 422 || msg.includes("already") || msg.includes("registered") || msg.includes("exists")) {
      console.error(`이미 존재하는 이메일입니다: ${email}. 다른 DEMO_ACCOUNT_EMAIL 을 사용하거나 Supabase Authentication > Users 에서 해당 사용자를 삭제한 뒤 다시 실행하세요.`);
    } else {
      console.error("Auth 사용자 생성 실패:", authErr.message);
    }
    process.exit(1);
  }
  ...
  if (!clientRow) {
    const { error: clientErr } = await supabase.from("clients").insert({
      ...
      kakao_chat_url: "https://open.kakao.com/o/example",
      enabled_modules: {
        overview: true,
        execution: true,
        calendar: true,
    ...
    if (clientErr) {
      console.error("데모 클라이언트 생성 실패:", clientErr.message);
      process.exit(1);
    }
    console.log("데모 클라이언트 생성됨:", DEMO_CLIENT_ID);
  }
  ...
  console.log("프로필 생성됨 (role=client, client_id=데모 클라이언트)");
  ...
  const week5 = toDate(new Date(now.getTime() - 35 * 86400000).toISOString());
  const week6 = toDate(new Date(now.getTime() - 42 * 86400000).toISOString());
  const week7 = toDate(new Date(now.getTime() - 49 * 86400000).toISOString());
  const week8 = toDate(new Date(now.getTime() - 56 * 86400000).toISOString());
  const week9 = toDate(new Date(now.getTime() - 63 * 86400000).toISOString());
  const week10 = toDate(new Date(now.getTime() - 70 * 86400000).toISOString());
  const week11 = toDate(new Date(now.getTime() - 77 * 86400000).toISOString());
  const week12 = toDate(new Date(now.getTime() - 84 * 86400000).toISOString());
  ...
  const lastMonthStart = toDate(new Date(now.getFullYear(), now.getMonth() - 1, 1).toISOString());
  ...
  else console.log("목업 actions 삽입 완료: 9건 (설명·링크·관련 프로젝트 포함)");
  ...
  else console.log("목업 reports 삽입 완료: 6건 (상세 요약 포함)");
  ...
  else console.log("목업 metrics 삽입 완료: 주별 12주 + 월별 6개월 (KPI 추이/카드용)");

  const { data: integrationsData, error: intErr } = await supabase
    .from("data_integrations")
    .insert([
      { client_id: DEMO_CLIENT_ID, platform: "naver_ads", display_name: "네이버 광고 (데모)", credentials: {}, config: {}, status: "active", created_by: userId },
      { client_id: DEMO_CLIENT_ID, platform: "google_analytics", display_name: "GA4 (데모)", credentials: {}, config: {}, status: "active", created_by: userId },
    ])
    .select("id, platform");
  if (intErr || !integrationsData?.length) {
    console.warn("data_integrations insert 경고:", intErr?.message ?? "no data");
  } else {
    const integrationIds = integrationsData as { id: string; platform: string }[];
    const naverId = integrationIds.find((i) => i.platform === "naver_ads")?.id;
    const gaId = integrationIds.find((i) => i.platform === "google_analytics")?.id;
    ...
    if (platformMetricsRows.length > 0) {
      ...
      else console.log("목업 platform_metrics 삽입 완료: 일자별", platformMetricsRows.length, "건 (성과 그래프용)");
    }
  }

  console.log("\n완료. 테스트 계정으로 로그인하여 개요/실행/캘린더/프로젝트/리포트에서 목업 데이터를 확인하세요.");
  console.log("  이메일:", email);
  console.log("  비밀번호:", password === DEFAULT_DEMO_PASSWORD ? "Admin123!" : "(환경 변수 값)");
}
